...
_doc_attr_idx  = doc_attr_df.set_index("doc")

# 1. COV_CONT_COLS: 連続医師属性を施設にマッピング (fac_to_doc 経由)
for _cont in COV_CONT_COLS:
    if _cont in _doc_attr_idx.columns:
        _cov[_cont] = pd.Series(fac_to_doc).map(_doc_attr_idx[_cont]).reindex(_cov.index)
    else:
        _cov[_cont] = np.nan
    print(f"  {_cont}: {_cov[_cont].notna().sum()} / {len(_cov)} 件")

# 2. COV_CAT_COLS: カテゴリ属性を one-hot 化
#    baseline_cat  → washout 期間売上から4カテゴリ計算 (inline)
#    施設属性      → fac_df (_fac_attr_idx) から取得
#    医師属性      → doc_attr_df (_doc_attr_idx) から fac_to_doc 経由
for _cat in COV_CAT_COLS:
    ...
    elif _cat in _doc_attr_idx.columns:
        _cat_ser = pd.Series(fac_to_doc).map(_doc_attr_idx[_cat]).reindex(_cov.index)
    else:
