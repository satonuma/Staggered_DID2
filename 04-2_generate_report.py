"""
===================================================================
HTML総合レポート生成 [ver2: 複数医師施設対応]
===================================================================
02-2/06-2/09-2の分析結果(JSON)と既存PNGを読み込み、
自己完結型HTMLレポートを生成する。
...
if os.path.exists(propensity_score_path):
    ...
    loaded_files.append("propensity_score_analysis_v2.json")
...
if INCLUDE_ONLY_RW:
    ...
    print(f"  [Step 3] RWフィルタ(fac_honin): {len(_target_honins)} 施設")
else:
    _target_honins = _single_honins
    print(f"  [Step 3] スキップ (全施設): {len(_target_honins)} 施設")
...
def create_consort_diagram(flow):
    ...
    total_docs = flow.get("total_doctors", 0)
    total_facs = flow.get("total_facilities", 0)
    is_v2 = flow.get("version") == "v2"
    if is_v2:
        n_multi_doc_info = flow.get("multi_doctor_facilities", 0)
        n_clean = total_facs
        n_after_step2 = flow.get("analysis_doctors", total_docs)
        n_washout = flow.get("washout_excluded_facilities", 0)
        n_late = 0
    else:
        ...
        n_multi_fac = flow.get("multi_fac_doctors", 0)
        n_clean = flow.get("clean_pairs", 0)
        n_after_step2 = flow.get("after_step2_doctors", n_clean)
        n_washout = flow.get("washout_excluded", 0)
        n_late = flow.get("late_excluded", 0)
    ...
    if is_v2:
        ...
        y -= sp
        draw_box(ax, cx, y, 4.2, 0.9,
                 f"[Step 3] RWフィルタ\n{n_clean}施設 / {n_after_step2}医師\n(複数医師施設含む: {n_multi_doc_info}施設)")
        draw_arrow_right(ax, cx + 2.1, y, ex - 1.5, y)
        draw_excluded(ax, ex, y, 2.8, 0.7,
                      f"非RW施設\n(全施設対象の場合0)")
        ...
        y -= sp
        draw_box(ax, cx, y, 4.2, 0.9,
                 f"[Step 1] 施設内医師数==1\n{n_single}施設 (fac_honin単位)")
        draw_arrow_right(ax, cx + 2.1, y, ex - 1.5, y)
        draw_excluded(ax, ex, y, 2.8, 0.7,
                      f"複数医師施設\n{n_multi_doc}施設 除外")

        # Step 2: doctor_attribute → 所属施設数==1 (honin粒度)
        draw_arrow(ax, cx, y - 0.45, cx, y - sp + 0.45)
        y -= sp
        draw_box(ax, cx, y, 4.2, 0.9,
                 f"[Step 2] 所属施設数==1\n{n_after_step2}医師")
...
<table>
  ...
  {% if flow.version == "v2" %}
  <tr>
    <td>[Step 3] RWフィルタ (ver2)</td>
    <td>RW施設のみ (全施設対象の場合スキップ)</td>
    <td>-</td>
    <td>{{ flow.total_facilities }}施設 / {{ flow.analysis_doctors }}医師<br>(複数医師施設: {{ flow.multi_doctor_facilities }}施設)</td>
  </tr>
  <tr>
